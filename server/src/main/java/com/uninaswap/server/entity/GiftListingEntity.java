...
public class GiftListingEntity extends ListingEntity {
    ...
    @OneToMany(mappedBy = "giftListing", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<ThankYouOfferEntity> thankYouOffers = new ArrayList<>();
    ...
    public boolean acceptThankYouOffer(String offerId) {
        for (ThankYouOfferEntity offer : thankYouOffers) {
            if (offer.getId().equals(offerId)) {
                offer.setStatus(ThankYouOfferStatus.ACCEPTED);
                return true;
            }
        }
        return false;
    ...
    public boolean declineThankYouOffer(String offerId) {
        for (ThankYouOfferEntity offer : thankYouOffers) {
            if (offer.getId().equals(offerId)) {
                offer.setStatus(ThankYouOfferStatus.DECLINED);
                return true;
            }
        }
        return false;
    ...
    public List<ThankYouOfferEntity> getThankYouOffers() {
