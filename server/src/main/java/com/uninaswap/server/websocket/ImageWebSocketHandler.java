...
public class ImageWebSocketHandler extends TextWebSocketHandler {
    ...
    public ImageWebSocketHandler(ImageService imageService) {
        this.imageService = imageService;
    }
    
    @Override
    public void handleTextMessage(@NonNull WebSocketSession session, @NonNull TextMessage message) throws Exception {
        System.out.println("SERVER RECEIVED IMAGE REQUEST: " + message.getPayload().substring(0, Math.min(100, message.getPayload().length())) + "...");
